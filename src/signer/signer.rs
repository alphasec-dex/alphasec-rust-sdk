...
use ethers::{
    abi::{Abi, Token}, contract::Contract, core::types::{transaction::eip2718::TypedTransaction, Address, Bytes, Eip1559TransactionRequest, U256, U64}, providers::{Middleware, Provider}, signers::{LocalWallet, Signer}, types::transaction::eip712::{TypedData as Eip712TypedData, Eip712}
};
use serde_json;
use std::{
...
impl AlphaSecSigner {
    ...
    pub async fn create_session_data(
        ...
        if self.config.l1_wallet.is_none() {
            return Err(AlphaSecError::invalid_parameter("L1 wallet is required for session operations"));
        }

        let session_addr = ethers::utils::to_checksum(&session_wallet.address(), None);

        // Create EIP-712 typed data and sign
        let typed_json = self.create_session_register_typed_data(&session_addr, timestamp_ms, expires_at);
        let l1_wallet = self.config.l1_wallet.as_ref()
            .ok_or_else(|| AlphaSecError::invalid_parameter("L1 wallet is required for session registration"))?;
        ...
        let signature_placeholder = l1_wallet.sign_hash(ethers::types::TxHash(digest))
            .map_err(|e| AlphaSecError::signer(&format!("Failed to sign EIP-712 digest: {}", e)))?;
        let signature_b64 = base64::engine::general_purpose::STANDARD.encode(signature_placeholder.to_vec());
    ...
    pub fn create_order_data(
        ...
        tracing::debug!("🔍 Order payload bytes: {:?}", String::from_utf8_lossy(&final_tx_bytes[1..]));
    ...
    pub async fn generate_alphasec_transaction(
        ...
        let tx = Eip1559TransactionRequest {
            ...
            to: Some(ALPHASEC_ORDER_CONTRACT_ADDR.parse::<Address>().unwrap().into()),
    ...
    pub async fn generate_deposit_transaction(
        ...
        let l1_wallet = self.config.l1_wallet.as_ref()
            .ok_or_else(|| AlphaSecError::invalid_parameter("L1 wallet is required for deposit operations"))?;
        ...
        if token_id == ALPHASEC_NATIVE_TOKEN_ID.to_string() {
            ...
            let inbox_address: Address = inbox_addr.parse()
                .map_err(|e| AlphaSecError::invalid_address(&format!("Invalid inbox address: {}", e)))?;
            
            let contract = Contract::new(inbox_address, abi, l1_provider.clone());
            
            // Get current nonce
            let l1_address: Address = self.l1_address().parse().unwrap();
            let nonce = l1_provider.get_transaction_count(l1_address, None)
                .await
                .map_err(|e| AlphaSecError::generic(&format!("Failed to get nonce: {}", e)))?;

            // Build transaction
            let tx = contract.method::<_, ()>("depositEth", ())
            ...
            let signature = l1_wallet_chain.sign_transaction(&tx).await
                .map_err(|e| AlphaSecError::signer(&format!("Failed to sign transaction: {}", e)))?;
            ...
            // ERC20 token deposit
            let token_l1_addr = token_l1_address
                .ok_or_else(|| AlphaSecError::invalid_parameter("token_l1_address is required for ERC20 tokens"))?;

            // Validate address format
            if !token_l1_addr.starts_with("0x") || token_l1_addr.len() != 42 {
                return Err(AlphaSecError::invalid_address("Invalid token_l1_address format"));
            }
            ...
            let erc20_abi: Abi = serde_json::from_str(ERC20_ABI)
                .map_err(|e| AlphaSecError::generic(&format!("Failed to parse ERC20 ABI: {}", e)))?;
            
            let token_address: Address = token_l1_addr.parse()
                .map_err(|e| AlphaSecError::invalid_address(&format!("Invalid token address: {}", e)))?;
            
            let erc20_contract = Contract::new(token_address, erc20_abi, l1_provider.clone());
            
            // Check allowance
            let gateway_address: Address = erc20_gateway_addr.parse()
                .map_err(|e| AlphaSecError::invalid_address(&format!("Invalid gateway address: {}", e)))?;
            
            let allowance: U256 = erc20_contract
                .method::<_, U256>("allowance", (self.l1_address().parse::<Address>().unwrap(), gateway_address))
                .map_err(|e| AlphaSecError::generic(&format!("Failed to create allowance method: {}", e)))?
                .call()
                .await
                .map_err(|e| AlphaSecError::generic(&format!("Failed to check allowance: {}", e)))?;
            ...
            if allowance < value_onchain_unit.into() {
                ...
                let approve_tx = erc20_contract
                    ...
                    .map_err(|e| AlphaSecError::generic(&format!("Failed to create approve method: {}", e)))?
                    .gas(1_000_000)
                    .nonce(nonce)
                    .gas_price(U256::from(l1_provider.get_gas_price().await.unwrap()))
                    .from(l1_address);
                ...
                let l1_wallet_chain = l1_wallet.clone();
                let signed_approve_tx = l1_wallet_chain.sign_transaction(&approve_tx).await
                    .map_err(|e| AlphaSecError::signer(&format!("Failed to sign approve transaction: {}", e)))?;
                
                let raw_approve_tx = approve_tx.rlp_signed(&signed_approve_tx);
                
                // Send approve transaction
                let _tx_hash = l1_provider.send_raw_transaction(Bytes::from(raw_approve_tx))
                    .await
                    .map_err(|e| AlphaSecError::generic(&format!("Failed to send approve transaction: {}", e)))?;
            ...
            let router_abi: Abi = serde_json::from_str(ERC20_ROUTER_ABI)
                .map_err(|e| AlphaSecError::generic(&format!("Failed to parse router ABI: {}", e)))?;
            
            let router_address: Address = erc20_router_addr.parse()
                .map_err(|e| AlphaSecError::invalid_address(&format!("Invalid router address: {}", e)))?;
            ...
            let tx = router_contract
                ...
                .map_err(|e| AlphaSecError::generic(&format!("Failed to create outboundTransfer method: {}", e)))?
                .value(value_eth)
                .gas(1_000_000)
                .nonce(nonce)
            ...
            let l1_wallet_chain = l1_wallet.clone();
            let signed_tx = l1_wallet_chain.sign_transaction(&tx).await
                .map_err(|e| AlphaSecError::signer(&format!("Failed to sign transaction: {}", e)))?;
    ...
    pub async fn generate_withdraw_transaction(
        ...
        if token_id == ALPHASEC_NATIVE_TOKEN_ID.to_string() {
            ...
            let abi: Abi = serde_json::from_str(L2_SYSTEM_ABI)
                .map_err(|e| AlphaSecError::generic(&format!("Failed to parse L2 System ABI: {}", e)))?;
            
            let system_address: Address = system_contract_addr.parse()
                .map_err(|e| AlphaSecError::invalid_address(&format!("Invalid system contract address: {}", e)))?;
            ...
            // Build transaction
            let l1_address: Address = self.l1_address().parse().unwrap();
            let call = contract.method::<_, ()>("withdrawEth", l1_address)
                .map_err(|e| AlphaSecError::generic(&format!("Failed to create withdrawEth method: {}", e)))?
                .value(value_onchain_unit)
                .gas(1_000_000)
                .nonce(nonce)
                .from(l1_address);
            ...
            let erc20_router_addr = crate::types::l2_contracts::ALPHASEC_GATEWAY_ROUTER_CONTRACT_ADDR;
            
            // Parse L2 ERC20 Router ABI and create contract instance
            let abi: Abi = serde_json::from_str(L2_ERC20_ROUTER_ABI)
                .map_err(|e| AlphaSecError::generic(&format!("Failed to parse L2 ERC20 Router ABI: {}", e)))?;
            ...
            // Build transaction
            let l1_address: Address = self.l1_address().parse().unwrap();
            let call = contract.method::<_, ()>("outboundTransfer", (
                token_address,
                l1_address,
                value_onchain_unit,
                data
...
mod tests {
    ...
    fn test_signer_creation() {
        ...
        assert_eq!(signer.l1_address(), "0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266");
    }
    ...
    fn test_session_register_typed_data() {
        ...
        assert_eq!(typed_data["domain"]["verifyingContract"], VERIFYING_CONTRACT);
    ...
    async fn test_generate_alphasec_transaction() {
        ...
        let result = signer.generate_alphasec_transaction(None, &data, None).await;
        assert!(result.is_ok());
