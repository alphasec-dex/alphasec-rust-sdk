...
fn round_to_precision(value: f64, precision: u64) -> f64 {
    let multiplier = 10_f64.powi(precision as i32);
    (value * multiplier).round() / multiplier
}

/// Normalize price and quantity values by rounding them to appropriate precision
/// 
...
pub fn normalize_price_quantity(price: f64, quantity: f64) -> Result<(f64, f64), AlphaSecError> {
    ...
    // Round price and quantity to the calculated precision
    let rounded_price = round_to_precision(price, price_precision);
    let rounded_quantity = round_to_precision(quantity, quantity_precision);
